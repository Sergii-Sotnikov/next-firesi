...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!verifyRes.ok) {
      ...
      return NextResponse.json({ error: "reCAPTCHA service error", detail: text }, { status: 502 });
    }
    ...
    if (!sendRes.ok) {
      ...
      return NextResponse.json({ error: "EmailJS request failed", detail: text }, { status: 502 });
    }
